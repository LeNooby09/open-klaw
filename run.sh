...
if [ "$BARE_METAL" = true ]; then
	...
	# Export host UID/GID so docker-compose.yml builds the container user with matching
	# UID/GID. This means directories created here (owned by host user) are automatically
	# writable inside the container — no chown or sudo needed.
	export OPENKLAW_UID="${OPENKLAW_UID:-$(id -u)}"
	export OPENKLAW_GID="${OPENKLAW_GID:-$(id -g)}"

	# Pre-create host-side data directories so Docker doesn't create them as root.
	# Because the container user's UID/GID matches the host user, these dirs are
	# writable inside the container without any ownership changes.
